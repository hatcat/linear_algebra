...
namespace std::experimental::la {
	...
	struct fixed_size_matrix {
		...
		using other_t = fixed_size_matrix<Scalar, OtherRow, OtherCol>;
...
template<class Scalar, size_t RowCount, size_t ColCount>
inline std::experimental::la::fixed_size_matrix<Scalar, RowCount, ColCount>::fixed_size_matrix(std::initializer_list<Scalar> il) noexcept
{
